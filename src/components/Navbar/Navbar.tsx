...
import { Nav_customer, NAV_ITEMS, Nav_product } from "@/api/fakeApi";
import ProductsMenuItems from "./ProductsMenuItems";
import CustomersMenuItem from "./CustomersMenuItem";
...
const Navbar = () => {
  ...
  const activeState = isScrolled || isHovered;
  ...
  const activeNavContent = NAV_ITEMS.find((nav) => nav.key === activeItem);
  const isSubmenuOpen = !!(activeItem && activeNavContent);
  ...
  return (
    <>
      ...
      />
      <div
        className="hidden md:flex fixed left-0 right-0 top-4 z-999 justify-center transition-all duration-500 max-w-360 px-3.75 m-auto text-white"
        onMouseLeave={() => {
          setActiveItem(null);
          setIsHovered(false);
        }}
      >
        ...
        >
          ...
          <div className="px-4 py-1">
            <div className="flex justify-between items-center transition-all duration-700 h-10">
              {/* Logo */}
              <div className="flex items-center gap-2 hover:opacity-80 cursor-pointer duration-200 w-50">
                <svg width="26px" viewBox="0 0 31 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                {!activeState && !activeItem &&
                  <svg className="h-2.5 lg:h-5.5" viewBox="0 0 132 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              {/* Nav Inner Items */}
              <div className={`flex items-center rounded-full transition-all duration-500 px-2.5
                ${!activeState && !activeItem && "bg-[#ffffff1a] backdrop-blur-[15px]"}`}>
                {NAV_ITEMS?.map((item) => (
                  <div
                    key={item.key}
                    onMouseEnter={() => {
                    ...
                    className={`text-xs lg:text-sm font-medium cursor-pointer p-2.5 transition-colors duration-200
                      ${activeItem === item.key ? "text-white" : "text-white/70 hover:text-white"}`}
                  >
                    {item.label}
                  </div>
                ))}
              ...
              <div className="hidden md:flex items-center gap-1">
                <Link href="/login" className="sonar-button text-[10px] lg:text-xs leading-1.2 font-medium flex items-center lg:gap-2 cursor-pointer h-full py-1 px-4 rounded-full">
                  SIGN IN <ArrowUpRight size={18} />
                </Link>
                <Link href="/#" className="sonar-button2 text-[10px] lg:text-xs leading-1.2 font-medium text-white py-1 px-4 rounded-full flex items-center lg:gap-2">
                  {activeState ? "SCHEDULE A DEMO" : "SIGN UP"} <ArrowUpRight size={18} />
                </Link>
              </div>
            </div>
          </div>
          {/* Expandable submenu */}
          <div className={`overflow-hidden transition-all ${isSubmenuOpen
